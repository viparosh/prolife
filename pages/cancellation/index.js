import { getCancelSchedule, cancelSchedule, sendSmsCancelNote } from '../../services/cancelappointment.services'
...
const cancellation = () => {	
  ...
  const cancelSched = async() => {
    ...
    const res = await cancelSchedule({
      ...
      subID: subID.current,
      cancellationCode: code.current.value,
    ...
    if(res.success){
      ...
      await uploadLog(
        { 
          ...
          content: `${info.firstName + ' ' + info.lastName} ( ${info.contact} ) cancelled her appoointment`,
  ...
  const validateField = async () => {
    setSuccessNotif(false)
    const newData = {cancellationCode:code.current.value,contact:number.current.value}
    const sched = await getCancelSchedule(newData)
  ...
  return (
    <NoLayout>
      <div className="text-sm sm:text-sm border rounded-md p-3">
        <Link href="../login">
          Back to Patient Portal
        </Link>
      </div>
      <div className="md:shrink w-full h-full flex-col flex py-10  items-center justify-center">
        <p className="text-2xl font-semibold mb-10">Cancel Appointment</p>
        <div className="md:w-1/4 w-1/2 grid gap-4 grid-rows-3">
        <input
            ref={number}
        ...
        />
          ...
          <div className="flex sm:flex-col grid gap-2 grid-cols-2">
            <button className="w-full bg-gray-600 text-white" onClick={validateField}>Submit</button>
            <button className="w-full bg-themeViolet text-white" onClick={clearFields}>Clear</button>
          </div>
        ...
        { table && 
            <div className="mt-5 border py-3 px-4 border-neutral-500 w-1/4">
              ...
              <p className="text-base">Time: {moment(info.time.from).format("h:mm a")} - {moment(info.time.to).format("h:mm a")} </p>
