...
const Sidebar = ({ role, active, modal, setModal, profile }) => {
  ...
  const logoutHandler = async () => {
    ...
    if (result.success) {
      await uploadLog({
        ...
        category:
          user.role == 'patient' ? 'Patient - Logout' : 'Staff - Logout',
        role: user.role,
  ...
  const linkLayout = (link) => {
    return link.map(({ icon, name, link }, index) => (
      ...
      >
        <Link href={link}>
          <div
            className={`items-cente mb-2 flex p-4 ${
              active == name && 'border border-white bg-[#625B7D]'
            } rounded-lg`}
          >
  ...
  return (
    <div
      ...
      } fixed top-0 z-20 h-screen min-w-full overflow-y-auto bg-[#625B7D] px-14 scrollbar-thin scrollbar-track-themeViolet scrollbar-thumb-white/30 md:sticky md:block md:min-w-sideBar md:translate-x-0`}
    >
